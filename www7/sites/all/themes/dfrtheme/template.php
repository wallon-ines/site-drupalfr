<?php
  ...
  function dfrtheme_preprocess_views_view(&$variables)
  {
    ...
    if($variables['view']->name == 'faq') {
      
      $faq_results = $variables['view']->result;
      $faq = array();
      
      if(!empty($faq_results)) {
        foreach($faq_results as $faq_key => $faq_result) {
          $category_id = $faq_result->field_field_categorie[0]['raw']['taxonomy_term']->tid;
          
          $faq[$category_id]['category_name'] = $faq_result->field_field_categorie[0]['raw']['taxonomy_term']->name;
          $faq[$category_id]['results'][$faq_key]['title'] = $faq_result->node_title;
          $faq[$category_id]['results'][$faq_key]['body'] = $faq_result->field_body[0]['raw']['safe_value'];
        }
        drupal_add_js(drupal_get_path('theme', 'dfrtheme').'/js/modernizr.js', array('type' => 'file', 'scope' => 'header'));
        drupal_add_js(drupal_get_path('theme', 'dfrtheme').'/js/faq.js', array('type' => 'file', 'scope' => 'footer'));
      }
      $variables['faq'] = $faq;
    }
  }
